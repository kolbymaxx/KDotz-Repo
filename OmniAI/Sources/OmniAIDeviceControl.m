...
static NSString *const kOAMCPProtocolVersion = @"2025-11-25";

@interface OmniAIDeviceControl ()
...
@implementation OmniAIDeviceControl

+ (instancetype)shared {
    static OmniAIDeviceControl *s; static dispatch_once_t once;
...
- (void)checkHealth:(void (^)(BOOL, NSString *))completion {
    OmniAIPreferences *p = [OmniAIPreferences shared];
    ...
    [[self.session dataTaskWithURL:url completionHandler:^(NSData *data, NSURLResponse *resp, NSError *err) {
        if (err || !data) {
            OAErr(@"MCP health check failed: %@", err.localizedDescription);
            if (completion) completion(NO, nil);
            return;
        }
        NSDictionary *json = [NSJSONSerialization JSONObjectWithData:data options:0 error:nil];
        BOOL ok = [json[@"status"] isEqualToString:@"ok"];
        OALog(@"MCP health: %@ (v%@)", ok ? @"ok" : @"bad", json[@"version"]);
...
- (NSMutableURLRequest *)requestWithBody:(NSDictionary *)body {
    NSMutableURLRequest *req = [NSMutableURLRequest requestWithURL:[OmniAIPreferences shared].mcpURL];
    req.HTTPMethod = @"POST";
    [req setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
    [req setValue:@"application/json, text/event-stream" forHTTPHeaderField:@"Accept"];
    [req setValue:kOAMCPProtocolVersion forHTTPHeaderField:@"MCP-Protocol-Version"];
...
- (void)sendMethod:(NSString *)method
    ...
    [[self.session dataTaskWithRequest:[self requestWithBody:body]
        ...
        NSDictionary *json = [self parseResponseData:data];
        if (!json) {
            if (completion) completion(nil, [NSError errorWithDomain:kOAIdentifier code:-1
                userInfo:@{NSLocalizedDescriptionKey: @"Unparseable MCP response"}]);
            return;
        }
        if (json[@"error"]) {
            NSString *msg = json[@"error"][@"message"] ?: @"MCP error";
            if (completion) completion(nil, [NSError errorWithDomain:kOAIdentifier code:-2
...
- (void)ensureInitialized:(void (^)(BOOL, NSError *))completion {
    ...
    NSDictionary *params = @{
        @"protocolVersion": kOAMCPProtocolVersion,
        @"capabilities": @{},
        @"clientInfo": @{ @"name": @"OmniAI", @"version": @"1.0.0" },
    };
    [self sendMethod:@"initialize" params:params completion:^(NSDictionary *result, NSError *error) {
        if (error) {
            OAErr(@"MCP initialize failed: %@", error.localizedDescription);
            if (completion) completion(NO, error);
            return;
        }
        self.initialized = YES;
        OALog(@"MCP initialized: %@", result[@"serverInfo"]);
...
- (void)describeScreenIfAvailable:(void (^)(NSString *))completion {
    [self checkHealth:^(BOOL ok, NSString *version) {
        if (!ok) { if (completion) completion(nil); return; }
        [self callTool:@"describe_screen" arguments:@{} completion:^(NSString *result, NSError *error) {
            if (completion) completion(error ? nil : result);
        }];
    }];
}
...
- (NSString *)flattenToolResult:(NSDictionary *)result {
    ...
    for (NSDictionary *block in content) {
        ...
        if ([type isEqualToString:@"text"]) {
            ...
            // Summarise — never inline raw base64 into the text tool channel.
